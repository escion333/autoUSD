...
contract YieldDistributorTest is Test {
    ...
    function setUp() public {
        ...
        yieldDistributor = new YieldDistributor(address(usdc), motherVault, treasury, INITIAL_MANAGEMENT_FEE_BPS);
        vm.stopPrank();
    }
    ...
    function test_Revert_WhenDeployWithZeroAddress() public {
        vm.expectRevert(abi.encodeWithSelector(YieldDistributor.InvalidAddress.selector, "Zero address provided"));
    ...
    function test_Revert_WhenDeployWithInvalidFee() public {
        vm.expectRevert(abi.encodeWithSelector(YieldDistributor.InvalidFee.selector, "Fee cannot exceed 100%"));
        new YieldDistributor(address(usdc), motherVault, treasury, 10_001);
    }
    ...
    function test_CalculateFees() public {
        ...
        uint256 expectedFee = (grossYield * INITIAL_MANAGEMENT_FEE_BPS) / 10_000;
    ...
    function test_RecordHarvest_Permissions() public {
        ...
        vm.expectRevert(
            abi.encodeWithSignature(
                "AccessControlUnauthorizedAccount(address,bytes32)", randomAddress, yieldDistributor.MOTHER_VAULT_ROLE()
            )
        );
        yieldDistributor.recordHarvest(1000e6, 101_000e6);
        vm.stopPrank();
    }
    ...
    function test_SetManagementFee_Revert_TooHigh() public {
        vm.prank(admin);
        vm.expectRevert(abi.encodeWithSelector(YieldDistributor.InvalidFee.selector, "Fee cannot exceed 100%"));
        yieldDistributor.setManagementFee(10_001);
    }
    ...
    function test_Fuzz_CalculateFees(uint96 _grossYield) public {
        ...
        uint256 expectedFee = (grossYield * yieldDistributor.managementFeeBps()) / 10_000;
