...
// Import tokens - we'll need to compile TypeScript first or use a different approach
const tokensSource = `
{
  "colors": {
    "primary": "#4CA8A1",
  ...
  "gradient": {
    "primary": "linear-gradient(135deg, #4CA8A1 0%, #D6C7A1 100%)",
    "soft": "linear-gradient(180deg, #FFFFFF 0%, #E8F6F5 100%)",
    "mist": "linear-gradient(180deg, #F9FBFB 0%, #EAEFF2 100%)",
    "hero": "linear-gradient(135deg, #4CA8A1 0%, #72D1CB 50%, #D6C7A1 100%)"
  },
  "animation": {
    "duration": {
      "instant": "0ms",
...
console.log('✅ Design tokens exported to public/design-tokens.json');

// Also generate a CSS file with all the tokens as CSS variables
const generateCSS = () => {
  let css = ':root {\n';
  ...
  // Gradients
  Object.entries(tokens.gradient).forEach(([key, value]) => {
...
const cssPath = path.join(__dirname, '..', 'public', 'design-tokens.css');
fs.writeFileSync(cssPath, generateCSS());

console.log('✅ CSS variables exported to public/design-tokens.css');