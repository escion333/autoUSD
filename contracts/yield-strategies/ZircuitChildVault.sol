    ...
    IMessageRecipient
{
    ...
    function _handleDeposit(uint256 amount) internal {
        ...
        uint[] memory amountsOut = zuitRouter.getAmountsOut(halfAmount, path);
        uint256 minTokenBOut = amountsOut[1] * (10000 - slippageBps) / 10000;

        zuitRouter.swapExactTokensForTokens(
            halfAmount,
            minTokenBOut,
            path,
            address(this),
        ...
        uint256 minTokenBForLP = tokenBBalance * (10000 - slippageBps) / 10000;
    ...
    function _withdrawFromPair(uint256 _pairId, uint256 _lpAmount) internal returns (uint256) {
        ...
        (uint256 usdcFromLp, uint256 tokenBFromLp) = zuitRouter.removeLiquidity(
            pair.tokenA,
            pair.tokenB,
            _lpAmount,
            0, 
        ...
        if (tokenBFromLp > 0) {
            ...
            uint[] memory amountsOut = zuitRouter.getAmountsOut(tokenBFromLp, path);
            uint256 minUsdcFromSwap = amountsOut[1] * (10000 - slippageBps) / 10000;

            uint[] memory receivedAmounts = zuitRouter.swapExactTokensForTokens(
                tokenBFromLp, 
                minUsdcFromSwap, 
                path, 
                address(this), 
    ...
    function addPair(address _tokenB, address _pairAddress, address _zuitRouter) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(pairIndex[_pairAddress] == 0, "Pair already exists");

        supportedPairs.push(PairInfo({
            tokenA: usdc,
            tokenB: _tokenB,
