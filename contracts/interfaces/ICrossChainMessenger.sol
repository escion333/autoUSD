...
interface ICrossChainMessenger {
    ...
    event MessageSent(uint32 indexed targetChainId, MessageType indexed messageType, bytes32 messageId, uint256 nonce);

    event MessageReceived(
        uint32 indexed sourceChainId, MessageType indexed messageType, bytes32 messageId, uint256 nonce
    );
