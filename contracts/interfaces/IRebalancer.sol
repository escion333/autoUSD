...
interface IRebalancer {
    ...
    event RebalanceTriggered(
        uint32 indexed fromChain, uint32 indexed toChain, uint256 indexed amount, uint256 apyDifferential
    );

    event RebalanceCompleted(uint32 indexed fromChain, uint32 indexed toChain, uint256 indexed amount, uint256 gasUsed);

    event MetricsUpdated(uint32 indexed chainId, uint256 indexed apy, uint256 indexed totalValue);

    event ConfigUpdated(string indexed parameter, uint256 indexed oldValue, uint256 indexed newValue);
    ...
    function emergencyRebalance(uint32 sourceChain, uint32 targetChain, uint256 amount) external;

    function emitRebalanceCompleted(uint32 sourceChain, uint32 targetChain, uint256 amount, uint256 gasUsed) external;
}
